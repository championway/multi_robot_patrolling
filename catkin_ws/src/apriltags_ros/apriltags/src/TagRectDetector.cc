...
namespace AprilTags {
  ...
  std::vector<TagDetection> TagRectDetector::extractTags(const cv::Mat& image, std::vector<cv::Rect>& quad_proposals) {
  ...
  for (unsigned int qi = 0; qi < quads.size(); qi++ ) {
	  ...
	  vertices.push_back(cv::Point(p1.first, p1.second));
      vertices.push_back(cv::Point(p2.first, p2.second));
  ...
  std::vector<TagDetection> TagRectDetector::extractTags(const cv::Mat& image) {

  std::vector<cv::Rect> quad_proposals;
  return this->extractTags(image, quad_proposals);
